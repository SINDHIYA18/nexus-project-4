...
function getCats()
{
    ...
    while ($row_cats = mysqli_fetch_array($run_cats)) {
        ...
        echo "<li><a href='index.php?cat=$cat_id' style='text-decoration: none; font-weight:bold;'>$cat_title</a></li>";
    }
}
...
function getTypes()
{
    ...
    while ($row_types = mysqli_fetch_array($run_types)) {
        ...
        echo "<li><a href='index.php?type=$type_id' style='text-decoration: none; font-weight:bold;'>$type_title</a></li>";
    }
}
    ...
    if (!isset($_GET['cat'])) {
        if (!isset($_GET['type'])) {
            ...
            while ($row_pack = mysqli_fetch_array($run_pack)) {
				...
				<a href='index.php?add_cart=$pack_id'><button style='float: right; font-size:14px; cursor: pointer; padding: 2px 4px; font-weight:bold;'>Book</button></a>
				</div>
				";
            }
        }
...
function getCatPack()
{
    if (isset($_GET['cat'])) {
        ...
        while ($row_cat_pack = mysqli_fetch_array($run_cat_pack)) {
			...
			<p><b> &#8377; $pack_price</b></p>
			<a href='details.php?pack_id=$pack_id' style='float: left;font-size:18px;text-decoration: none; font-weight:bold;'>Details</a>
			<a href='cart.php?pack_id=$pack_id'><button style='float: right; font-size:14px; cursor: pointer; padding: 2px 4px; font-weight:bold;'>Book</button></a>
			</div>
			";
        }
    }
